...
fun enlargedSearchScope(searchScope: GlobalSearchScope, module: Module): GlobalSearchScope {
    return KotlinResolveScopeEnlarger.EP_NAME.extensions.fold(searchScope) { scope, enlarger ->
        val extra = enlarger.getAdditionalResolveScope(module)
        if (extra != null) scope.union(extra) else scope
    }
}

abstract class KotlinResolveScopeEnlarger {
    abstract fun getAdditionalResolveScope(module: Module): SearchScope?
